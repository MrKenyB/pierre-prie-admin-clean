...
import { Plus, Trash2, Edit, ArrowRight, Upload, X, Loader2, Eye } from 'lucide-react';
...
const Formations = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        const response = await axios.put(`${backendUrl}/api/parcours/update/${editingId}`, data, {
          headers: {
  ...
  const resetForm = () => {
    setFormData({ titre: '', classe: '', description: '', imageFile: null });
    setPreview(null);
    setAptitudes([]);
    setDebouches([]);
    setAptitudeInput('');
              ...
              <div key={formation._id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition overflow-hidden border">
                ...
                <div className="p-4">
                  <div className="mb-3">
                    ...
                    <p className="text-sm text-slate-600 line-clamp-2">{formation.description}</p>
                  </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center z-10">
                <div>
                  <h3 className="text-2xl font-bold text-slate-900">{selectedFormation.titre}</h3>
                  {selectedFormation.classe && (
                    <p className="text-sm text-emerald-600 font-semibold">Classe: {selectedFormation.classe}</p>
                  )}
                  <p className="text-sm text-slate-500">Détails complets de la formation</p>
                </div>
                <button 
                ...
                {selectedFormation.aptitude.length > 0 && (
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2 pl-4">
                      {selectedFormation.aptitude.map((apt, index) => (
                        <span 
                          key={index} 
                ...
                {selectedFormation.debouche.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2 pl-4">
                      {selectedFormation.debouche.map((deb, index) => (
                        <div 
                          key={index} 
                        ...
                        >
                          ...
                          <span className="text-slate-700">{deb}</span>
                        </div>
                      ))}
                    </div>
                  </div>
